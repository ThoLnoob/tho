-- Tho Lnoob Galaxy Hub - Complete LocalScript
...
-- root gui (PlayerGui ensures works in Studio/test)
...
local ToggleButton = new("ImageButton", {
    ...
    Image = "rbxassetid://89300403770535",
...
local TitleLabel = new("TextLabel", {
    Parent = TopBar, Text = "Tho Lnoob", TextColor3 = TEXT, BackgroundTransparency = 1,
...
local Subtitle = new("TextLabel", {
    Parent = TopBar, Text = "Galaxy Hub", TextColor3 = ACCENT2, BackgroundTransparency = 1,
...
new("UIListLayout", {Parent = ContentPane}).Padding = UDim.new(0,8)
...
local function selectTab(name)
    for k,btn in pairs(tabButtons) do
        if k == name then btn.BackgroundColor3 = ACCENT; btn.TextColor3 = Color3.new(0,0,0)
        else btn.BackgroundColor3 = Color3.fromRGB(18,10,30); btn.TextColor3 = TEXT end
    end
...
-- helpers for rows, switches, plus/minus
...
local function createSwitch(parent, initial)
    ...
    return {
        ...
        OnChanged = function(fn) table.insert(listeners, fn) end
    }
end
...
local function createPlusMinus(parent, default, minV, maxV)
    ...
    local function set(v)
        ...
        for _,fn in ipairs(listeners) do pcall(fn, val) end
    end
...
espSwitch.OnChanged(function(state)
    espEnabled = state
    if espEnabled then
        for _,p in pairs(Players:GetPlayers()) do if p.Character then createESPForPlayer(p) end end
        Players.PlayerAdded:Connect(function(p) p.CharacterAdded:Connect(function() if espEnabled then createESPForPlayer(p) end end) end)
        for _,p in pairs(Players:GetPlayers()) do p.CharacterAdded:Connect(function() if espEnabled then createESPForPlayer(p) end end) end
    else
        removeAllESP()
    end
end)

-- Player tab (rejoin, reset)
...
-- toggle menu function & keybind (M)
...
UserInputService.InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.M then toggleMenu() end
end)

-- Character respawn handling: apply values and ensure fly off
...
-- ensure no dangling when script removed
script.AncestryChanged:Connect(function(_, parent)
    if not parent then cleanup() end
end)

print("[Tho Lnoob Galaxy Hub] Loaded successfully ✅")
